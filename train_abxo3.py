...
def train_epoch(model, loader, criterion, optimizer, use_cuda=False, if_return=False):
    model.train()
    running_loss = 0.0
    tic = timer()
    result=[]
    ...
    if if_return:
        pred=[i[0] for i in result]
        targ=[i[1] for i in result]
        return torch.cat(pred), torch.cat(targ), epoch_loss
    return epoch_loss
...
def main(n):
    ...
    print(args)
    raw_data_path = args.raw_datadirs[2]
    cache_path=args.datasetdir+args.datasets[2]+'_'+args.target[n]+'/'
    rules_path=args.rule_dirs[2]+args.target[n]+'/'
    ...
    train_set,valid_set,test_set=split_dataset(raw_data_path,args.seed,[0.85,0.15,0.05])
    train_dset = GraphDataset(cif_dir=raw_data_path,data_dir=train_set,cache_path=cache_path+'train//',max_num_nbr=max_num_nbr,target_prop_files=targdatas,dis=dis,num_workers=None,if_cache_files=True,target=args.target[n],index_file=args.index_dirs[2])
    train_loader = DataLoader(train_dset, batch_size=args.batch_size, shuffle=True, pin_memory=True,collate_fn=batch_func)
    
    val_dset = GraphDataset(cif_dir=raw_data_path,data_dir=valid_set,cache_path=cache_path+'val//',max_num_nbr=max_num_nbr,target_prop_files=targdatas,dis=dis,num_workers=None,if_cache_files=True,target=args.target[n],index_file=args.index_dirs[2])
    val_loader = DataLoader(val_dset, batch_size=args.batch_size, shuffle=False, pin_memory=True,collate_fn=batch_func)
    
    test_dset = GraphDataset(cif_dir=raw_data_path,data_dir=test_set,cache_path=cache_path+'test//',max_num_nbr=max_num_nbr,target_prop_files=targdatas,dis=dis,num_workers=None,if_cache_files=True,target=args.target[n],index_file=args.index_dirs[2])
    ...
    model = GraphTransformer(edge_dim=args.edge_dim,
                             ...
                             rangefiles=args.rangefiles[2],
                             factor=args.factors[n]) 
    ...
    for epoch in range(epoch_s, args.epochs):
        ...
        train_loss = train_epoch(model, train_loader, criterion, optimizer, args.use_cuda, if_return)
