...
def main(n):
    ...
    print(args)
    raw_data_path = args.raw_datadirs[0]
    cache_path=args.datasetdir+args.datasets[0]+'_'+args.target[n]+'/'
    rules_path=args.rule_dirs[0]+args.target[n]+'/'
    ...
    train_set,valid_set,test_set=split_dataset(raw_data_path,args.seed,[0.85,0.1,0.05])
    train_dset = GraphDataset(cif_dir=raw_data_path,data_dir=train_set,cache_path=cache_path+'train//',max_num_nbr=max_num_nbr,target_prop_files=targdatas,dis=dis,num_workers=None,if_cache_files=True,target=args.target[n],index_file=args.index_dirs[0],if_abo3_system=True)
    train_loader = DataLoader(train_dset, batch_size=args.batch_size, shuffle=False, pin_memory=True,collate_fn=batch_func)
    
    val_dset = GraphDataset(cif_dir=raw_data_path,data_dir=valid_set,cache_path=cache_path+'val//',max_num_nbr=max_num_nbr,target_prop_files=targdatas,dis=dis,num_workers=None,if_cache_files=True,target=args.target[n],index_file=args.index_dirs[0],if_abo3_system=True)
    val_loader = DataLoader(val_dset, batch_size=args.batch_size, shuffle=False, pin_memory=True,collate_fn=batch_func)
    
    test_dset = GraphDataset(cif_dir=raw_data_path,data_dir=test_set,cache_path=cache_path+'test//',max_num_nbr=max_num_nbr,target_prop_files=targdatas,dis=dis,num_workers=None,if_cache_files=True,target=args.target[n],index_file=args.index_dirs[0],if_abo3_system=True)
